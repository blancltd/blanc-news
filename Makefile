...
isort-lint: isort-version
	isort --recursive --check-only --diff glitter_news

isort-format: isort-version
	isort --recursive glitter_news
...
# YAPF
yapf-lint:
	yapf_lint_output="`yapf -r -p -d --exclude="*/migrations/*.py" --style .style.yapf glitter_news`" && \
	if [[ $$yapf_lint_output ]]; then echo -e "$$yapf_lint_output"; exit 1; fi

yapf-format:
	yapf -r -i -p --exclude="*/migrations/*.py" --style .style.yapf glitter_news
